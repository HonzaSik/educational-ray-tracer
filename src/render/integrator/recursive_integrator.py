...
class RecursiveIntegrator(Integrator):
    ...
    def cast_ray(self, ray: Ray, depth: int | None = None) -> Color:
        ...
        if reflectivity > 0.0 and transparency > 0.0:
            ...
            kr = fresnel_reflectance(ray.direction, n_shade, ior_out=ior_out, ior_in=ior_in)

            result = local_color * (1.0 - kr) * (1.0 - transparency) + reflected_color * kr + refracted_color * (1.0 - kr) * transparency
